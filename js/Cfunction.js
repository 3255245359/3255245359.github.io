...
/* 	  var username=getCookie("username");
	  if (username!="")
	  {
	    $("dName").innerText = getCookie("username");
	
	  } */
	  // function yk(){ if(localStorage.getItem("myname")==null){
		 //  $("dName").innerText="游客";
		 //  }
		 //  else{
			//   $("dName").innerText =localStorage.getItem("myname");
		 //  }
	  // }
var $User = function() {
    ...
    return {
        ...
        Server: {
            List: [["1号(深圳电信)", "http://pvz1.lonelystar.org/", "NaN"], ["2号(浙江联通)", "http://pvz2.lonelystar.org/", "NaN"], ["3号(浙江电信)", "http://pvz3.lonelystar.org/", "NaN"], ["4号(浙江电信)", "http://pvz4.lonelystar.org/", "NaN"]],
            SpeedURL: "",
            DataURL: "",
            SpeedInx: 0,
            DataInx: 0,
...
oP = {
    Init: function(a) {
        ...
        a && a.FlagNum ? (r.FlagHeadStep = Math.floor(140 / (a.FlagNum - 1)), r.MonPrgs = function() {
            var u = oP,
            j, i = u.FlagZombies,
            s, t, f = $User.Visitor; ! --u.NumZombies && (i < u.FlagNum ? (u.ReadyFlag = ++i, oSym.addTask(500, u.FlagPrgs, [])) : (u.FlagToEnd(), $User.isAuthorWebsite && $User.Visitor.UserName != "游客" && (ClearChild($("JSPVZAjax")), f.SaveLvl && NewEle("JSPVZAjax", "script", 0, {
                src: $User.Server.DataURL + "asp/UserSave.asp?Lvl=" + (s = oS.LvlEName) + "&T=" + (oSym.Now - oS.StartTime),
                type: "text/javascript"
            },
...
InitHandBookPCard = function() {
    ...
    var d = "",
    g, f, e = [oPeashooter, oSunFlower, oCherryBomb, oWallNut, oPotatoMine, oSnowPea, oChomper, oRepeater, oPuffShroom, oSunShroom, oFumeShroom, oGraveBuster, oHypnoShroom, oScaredyShroom, oIceShroom, oDoomShroom, oLilyPad, oSquash, oThreepeater, oTangleKlep, oJalapeno, oSpikeweed, oTorchwood, oTallNut, oSeaShroom, oSplitPea, oStarfruit, oPumpkinHead, oFlowerPot, oCoffeeBean, oGarlic, oGatlingPea, oGloomShroom, oTwinSunflower, oSpikerock],
    a = e.length,
    b = 0,
    c;
    while (b < a) {
...
InitHandBookZCard = function() {
    PlayAudio("gravebutton");
    var d = "",
    g, f, e = [oZombie, oFlagZombie, oConeheadZombie, oPoleVaultingZombie, oBucketheadZombie, oNewspaperZombie, oScreenDoorZombie, oFootballZombie, oDancingZombie, oBackupDancer, oDuckyTubeZombie1, oSnorkelZombie, oZomboni, oDolphinRiderZombie, oImp, oJackinTheBoxZombie],
    a = e.length,
    b = 0,
    c;
    while (b < a) {
...
WordUTF8 = '<div id="dLogo" style="position:absolute;width:900px;height:600px;z-index:1"><div id="LogoWord" style="position:absolute;color:#FF0;top:300px;width:100%;height:100px"><span style="position:absolute;width:205px;height:130px;left:375px;top:175px;cursor:pointer" onclick="PlayAudio(\'gravebutton\');SetBlock($(\'dSurface\'),$(\'iSurfaceBackground\'));ShowNameDiv()"></span><div style="position:absolute;font-size:14px;left:660px;text-align:center;width:140px;top:185px;line-height:1.5;font-weight:bold"><span style="cursor:pointer"><span id="sTime" style="cursor:pointer"></span></span></div></div></div>',
...
ShowLoginDiv = function() {
    $User.isAuthorWebsite ? (PlayAudio("tap"), SetBlock($("dLogin"))) : (PlayAudio("tap"), SetBlock($("dLogin"))) //GotoAuthorWebsite("登录和保存功能")
},
GameLogin = function(h) {
	...
	$("dName").innerText = $("txtName").value.toString();
	a=  $("txtName").value.toString();
    localStorage.myname = a;
/*    NewEle("JSPVZAjax", "script", 0, {
 ...
 GameLogout = function() {
    ...
    a.UserName =localStorage.getItem('myname') ;
    a.UserAuthority = 0;
    a.Progress = 1;
    $("dName").innerHTML = localStorage.getItem('myname');
...
CheckLogin = function() {
    ...
    // b = /^[\u4e00-\u9fa5\w]{3,10}$/,
    // d = /^\w{3,20}$/;
    return /* (a.exec(c) || b.exec(c)) && d.exec(e) ? */true;
}, 
...
LoadLvl = function(e, c) {
    ...
    oSym.Init(function(g, f) { (f = $("JSPVZ")) && ClearChild(f);
        NewEle("JSPVZ", "script", 0, {
            src: "level/" + (oS.Lvl = localStorage.getItem("mylv")==null || g==0 ? g : (g==1 ? localStorage.getItem("mylv")  :g))+".js" ,
            type: "text/javascript",
        },
...
function(b) {
    ...
    if (a) {
        ...
        setTimeout(a.play(),1500)
    } else {
        NewMusic(b);
       setTimeout(oAudio[b].play(),1500)
    }
}: function(a) {
    NewMusic(a)
},
